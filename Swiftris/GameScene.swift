...
class GameScene: SKScene {
    ...
    override init(size: CGSize) {
        ...
        let gameBoard = SKSpriteNode(texture: gameBoardTexture, size: CGSizeMake(BlockSize * CGFloat(Swiftris.numColumns), BlockSize * CGFloat(Swiftris.numRows)))
        ...
        self.runAction(SKAction.repeatActionForever(SKAction.playSoundFileNamed("theme.mp3", waitForCompletion: true)))
    }
    ...
    func playSound(sound:String) {
        self.runAction(SKAction.playSoundFileNamed(sound, waitForCompletion: false))
    }
    
    override func update(currentTime: CFTimeInterval) {
        /* Called before each frame is rendered */
        if self.lastTick == nil {
            return
        }
        let timePassed = self.lastTick!.timeIntervalSinceNow * -1000.0
    ...
    func pointForColumn(column: Int, row: Int) -> CGPoint {
        let x: CGFloat = self.LayerPosition.x + (CGFloat(column) * BlockSize) + (BlockSize / 2)
        let y: CGFloat = self.LayerPosition.y - ((CGFloat(row) * BlockSize) + (BlockSize / 2))
